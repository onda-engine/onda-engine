...
engine (`@onda-engine/react` + `@onda-engine/components` + `@onda-engine/player` + the native
...
- **ondajs — removable after a small step.** Studio imports only `manifest` from
  ...
  1:1 in `@onda-engine/components`. Blockers: a runtime component **manifest** (Studio's
...
- **Remotion — phased, not a swap.** `@onda-engine/react` already covers the
...
| --- | --- | --- |
| `remotion` core | ~269 import sites; top: useVideoConfig, useCurrentFrame, interpolate, AbsoluteFill, spring | ✅ ~70% covered by `@onda-engine/react`; gaps: Audio/Video primitives, `interpolateColors` (now added), `registerRoot`/`getInputProps`/`staticFile` (bundler-only, N/A) |
...
| `@remotion/player` | editor preview (scrub/controls/seek/ref) | ⚠️ `@onda-engine/player` exists; needed imperative ref/events/initialFrame (now added) |
...
| `ondajs` | `manifest` only; DURATION + choreography re-exports | ✅ DURATION/choreography in `@onda-engine/components`; ❌ runtime manifest |

## Long poles (the real blockers)

1. **`@onda-engine/player` imperative API** — host-drivable preview (seekTo/play/pause,
...
1. **Quick wins** *(small)* — `interpolateColors` in `@onda-engine/react`; verify
   `DURATION`/choreography parity; `Img`→`Image` alias. Unblocks dropping
   `ondajs/motion`. *(Done.)*
2. **`@onda-engine/player` imperative API** *(medium)* — `packages/player`. Unblocks
...
5. **Audio in the render graph** *(medium)* — `scene-rs` Audio node + `cli-rs`
   mux + `@onda-engine/components` AudioClip export path.
...
7. **Runtime component manifest** *(medium)* — code-generate from
   `@onda-engine/components` (name/category/title/description/schema) for the agent
   prompt + catalog.
...
- **zod version:** Studio aliases zod to v3 for its render bundle; verify
  `@onda-engine/components` schema/zod compatibility before publishing the manifest.

## Status

Done:
- `@onda-engine/react`: `interpolateColors` (+ unit tests); `Img` alias of `Image`.
- `@onda-engine/player`: imperative `ref` (`PlayerHandle`: seekTo/play/pause/toggle/
...
- `@onda-engine/react` transitions: `flip`/`clockWipe`/`iris`/`none` presentations
...
- **Render bridge (the Chromium-removal payoff):** `onda-cli` mp4 export now
  ...
  (c) emits per-frame `[onda-progress]` JSON (`--progress`). New **`@onda-engine/render`**
