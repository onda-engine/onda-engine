...
function Shot2A({ width, height }) {
  ...
  const total = nChars > 0 ? measureText(substr, SIZE, { fontFamily: SANS, fontWeight: WEIGHT }).width : 0
  ...
  const charXScale = nChars > 0 && CHAR_X[nChars - 1]
    ? total / (CHAR_X[nChars - 1].x + CHAR_X[nChars - 1].adv)
    : 1
  ...
  const nodes = [
    h(Text, { key: 'line', x: Math.round(startX), y: cyText, fontSize: SIZE, fontFamily: SANS, fontWeight: WEIGHT, color: '#3a3a3a', runs }, ''),
  ]
  ...
  nodes.push(h(Rect, { key: 'caret', x: Math.round(cursorX) + 2, y: cursorY, width: 10, height: cursorH, fill: '#F08030', opacity: blink }))
  ...
  return h(
    ...
    ...nodes,
    h(AudioClip, { key: 'audio', src: '/Users/rodrigosilva/dev/onda-engine/refs/shot2a-audio.aac', volume: 1 }),
  )
}

export default function flowsShot2A({ fps, durationInFrames, width, height }) {
  return h(Composition, { width, height, fps, durationInFrames, linear: true }, h(Shot2A, { width, height }))
}
