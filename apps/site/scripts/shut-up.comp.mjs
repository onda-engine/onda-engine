...
import { Composition, Group, Image, Text, useCurrentFrame, interpolate, Easing } from '@onda/react'
...
function Scene() {
  ...
  const s = interpolate(f, [0, 18], [0.86, 1], { easing: Easing.easeOutCubic, extrapolateLeft: 'clamp', extrapolateRight: 'clamp' })
  ...
  return h(
    ...
    h(
      ...
      h(Text, { x: 20, y: 150, fontSize: 132, fontWeight: 900, color: '#0f1216', letterSpacing: -4 }, 'SHUT'),
      h(Text, { x: 20, y: 292, fontSize: 132, fontWeight: 900, color: '#0f1216', letterSpacing: -4 }, 'UP'),
    ),
...
export default function shutUp({ fps }) {
  return h(Composition, { width: W, height: H, fps, durationInFrames: 40, linear: false }, h(Scene, null))
}
