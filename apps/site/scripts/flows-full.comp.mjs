...
function Pill({ x, y, label }) {
  ...
  return h(
    ...
    h(
      Text,
      { x: x + 12, y: y + 7, fontSize: 14, fontFamily: SANS, fontWeight: 500, color: WHITE },
      label,
    ),
  )
}
...
const S1_CW = 898,
  S1_CH = 253
const S1_CCX = W / 2,
  S1_CCY = CY // card centre — true vertical centre
...
const S1_FX = S1_CX + 54,
  S1_FW = S1_CW - 54 - 18
const S1_FY = S1_CY + 74,
  S1_FH = 72
...
function SceneShot1({ frame }) {
  ...
  const easeOut3_ = (x) => 1 - (1 - clamp(x, 0, 1)) ** 3
  ...
  return h(
    Group,
    { grain: { intensity: 0.05, size: 1.1, seed: frame }, opacity: toOut },
    h(Rect, {
      x: 0,
      y: 0,
      width: W,
    ...
    h(FadeType, {
      ...
      center: true,
      canvasW: W,
      y: Math.round(S1_CCY - TSIZE * 0.55),
      cursor: true,
    }),
    ...
    h(
      ...
      h(Rect, {
        ...
        width: cardW,
        height: cardH,
        cornerRadius: Math.min(26, cardH / 2),
        fill: WHITE,
      ...
      h(
        ...
        h(
          Text,
          { x: S1_FX, y: S1_CY + 24, fontSize: 28, fontFamily: SANS, fontWeight: 600, color: FG },
          'Flows Agent',
        ),
        ...
        h(Path, {
          ...
          stroke: slideP > 0.1 ? WHITE : '#3A3A3A',
          strokeWidth: 2.6,
          strokeCap: 'round',
          strokeJoin: 'round',
        }),
...
function SceneMacro({ frame }) {
  ...
  const total =
    nChars > 0 ? measureText(substr, 290, { fontFamily: SANS, fontWeight: 400 }).width : 0
  ...
  const runs = Array.from({ length: nChars }, (_, i) => ({
    text: TYPE_LINE[i],
    color: heat(cursorX - (startX + (CHAR_X[i].x + CHAR_X[i].adv) * scale)),
  }))
...
const LOG_ICONS = [
  {
    label: 'Image',
    desc: 'hero concept shot of a trail running shoe',
    cc: '#C2DDF8',
    ic: '#3A78C9',
    type: 'img',
    f: S_LOG + 8,
  ...
  {
    label: 'Video',
    desc: 'product ad animated from the image (trail env.)',
    cc: '#CCBBEE',
    ic: '#7B42BC',
    type: 'vid',
    f: S_LOG + 48,
  ...
  {
    label: 'TTS',
    desc: 'punchy voiceover line wired to composition',
    cc: '#F2BEFA',
    ic: '#A040B0',
    type: 'aud',
    f: S_LOG + 88,
...
function SceneLog({ frame }) {
  ...
  const iconEls = LOG_ICONS.map((item, i) => {
    const nextF = LOG_ICONS[i + 1]?.f ?? Number.POSITIVE_INFINITY
  ...
  const arcEnd = {
    x: SCX + SR * Math.sin((Math.PI * 2) / 3),
    y: SCY - SR * Math.cos((Math.PI * 2) / 3),
  }
  ...
  const spinnerEl = h(
    ...
    h(
      Group,
      { rotation: rotDeg, originX: SCX, originY: SCY },
      h(Path, {
        x: 0,
        y: 0,
...
function SceneSelector({ frame }) {
  ...
  const scrollY = spring({
    frame: Math.max(0, frame - S_SELECT),
    fps: FPS,
    from: SEL * ITEM_H,
    to: 0,
    config: SLOT_CFG,
...
function VideoCard({ frame }) {
  ...
  return h(
    ...
    h(
      Text,
      { x: x + w - 20, y: y - 8, fontSize: 12, fontFamily: SANS, fontWeight: 700, color: WHITE },
      '▶',
    ),
  )
}
...
function AudioCard({ frame }) {
  ...
  const WH = [
    6, 10, 16, 22, 14, 26, 18, 10, 22, 28, 14, 20, 8, 18, 24, 12, 20, 14, 22, 10, 18, 24, 8, 14, 20,
    16, 12, 20, 14, 18,
  ]
  const wfW = 200,
    wfH = 28,
  ...
  return h(
    ...
    h(
      Text,
      { x: x + 22, y: y + 18, fontSize: 14, fontFamily: SANS, fontWeight: 700, color: FG },
      '▶',
    ),
    h(
      Text,
      { x: x + 54, y: y + 22, fontSize: 11, fontFamily: SANS, fontWeight: 400, color: '#9CA3AF' },
      '0:13',
    ),
    ...WH.map((barH, i) => {
    ...
    h(
      Text,
      { x: x + 44, y: y + 68, fontSize: 12, fontFamily: SANS, fontWeight: 500, color: FG },
      'Victoria - Warm, Trustworthy, and Relatable',
    ),
    h(
      Text,
      { x: x + 14, y: y + 100, fontSize: 11, fontFamily: SANS, fontWeight: 400, color: '#6B7280' },
      'Built for the path ahead. From forest trails to everyday',
    ),
    h(
      Text,
      { x: x + 14, y: y + 116, fontSize: 11, fontFamily: SANS, fontWeight: 400, color: '#6B7280' },
      'miles, these trainers move with you.',
    ),
...
function SceneNodes({ frame }) {
  const zoom = spring({
    frame: Math.max(0, frame - S_NODES),
    fps: FPS,
    from: 1.05,
    to: 0.88,
    config: SPRING_SMOOTH,
    durationInFrames: S_HERO - S_NODES,
  })
  ...
  return h(
    ...
    h(
      ...
      h(Rect, {
        ...
        fill: WHITE,
        cornerRadius: 16,
        shadow: { color: '#00000020', blur: 30, offsetY: 10 },
      }),
      h(
        Text,
        { x: cx2 + 20, y: cy2 + 18, fontSize: 20, fontFamily: SANS, fontWeight: 600, color: FG },
        'Flows Agent',
      ),
      h(Ellipse, {
        x: cx2 + 20 + textW('Flows Agent', 20, 600) + 10,
        y: cy2 + 26,
        width: 10,
        height: 10,
        fill: GREEN,
...
function SceneThink({ frame }) {
  ...
  return h(
    ...
    h(
      ...
      h(
        Text,
        { x: HX, y: HY, fontSize: 34, fontFamily: SANS, fontWeight: 700, color: FG },
        'Flows Agent',
      ),
    ...
    h(
      ...
      ...RES_OPTS.map((lbl, i) => {
        ...
        return h(
          ...
          h(
            Text,
            { x: cx + 20, y: cy + 15, fontSize: 22, fontFamily: SANS, fontWeight: 500, color: FG },
            lbl,
          ),
        )
      }),
...
// ════════════════════════════════════════════════════════════════════════════
const WF_H = [
  6, 12, 18, 26, 16, 28, 22, 14, 20, 28, 16, 22, 10, 24, 28, 14, 20, 10, 24, 18, 26, 12, 18, 28, 8,
  16, 22, 18, 12, 20,
]
...
function SceneAudio({ frame }) {
  ...
  return h(
    ...
    h(
      ...
      h(
        Text,
        { x: AX + 24, y: AY + 20, fontSize: 20, fontFamily: SANS, fontWeight: 700, color: FG },
        '▶',
      ),
      h(
        Text,
        { x: AX + 66, y: AY + 24, fontSize: 16, fontFamily: SANS, fontWeight: 400, color: FG },
        '0:13',
      ),
      // Waveform bars
      ...WF_H.map((bh, i) => {
      ...
      h(
        Text,
        { x: AX + 22, y: AY + 154, fontSize: 16, fontFamily: SANS, fontWeight: 400, color: FG },
        'Built for speed and endurance. From first stride to final',
      ),
      h(
        Text,
        { x: AX + 22, y: AY + 176, fontSize: 16, fontFamily: SANS, fontWeight: 400, color: FG },
  ...
  const cards = MATS.map(([wx, wy, mw, mh], i) => {
    ...
    return h(
      ...
      h(Rect, {
        ...
        fill: WHITE,
        cornerRadius: 8,
        shadow: { color: '#00000012', blur: 12, offsetY: 5 },
      }),
      h(Image, {
        x: wx + 3,
        y: wy + 3,
        width: mw - 6,
        height: mh - 24,
        src: MAT_IMGS[i % MAT_IMGS.length],
        cornerRadius: 5,
      }),
      h(
        Text,
...
const ICON_DOTS = [
  { rx: 0.1, ry: 0.18, icon: '↩' },
  { rx: 0.78, ry: 0.12, icon: '▶' },
  { rx: 0.88, ry: 0.38, icon: '◫' },
  { rx: 0.06, ry: 0.52, icon: 'T' },
  { rx: 0.22, ry: 0.72, icon: '⊟' },
  { rx: 0.58, ry: 0.8, icon: '▷' },
  { rx: 0.8, ry: 0.74, icon: '▶' },
  { rx: 0.44, ry: 0.14, icon: '≡' },
  { rx: 0.92, ry: 0.6, icon: '↗' },
  { rx: 0.16, ry: 0.86, icon: 'Q' },
  { rx: 0.68, ry: 0.44, icon: 'D' },
  { rx: 0.35, ry: 0.48, icon: '⊞' },
  { rx: 0.55, ry: 0.25, icon: '≡' },
  { rx: 0.72, ry: 0.6, icon: '↩' },
]
const WHAT_CARD_W = 300,
  WHAT_CARD_H = 80
...
function SceneFlood({ frame }) {
  ...
  return h(
    Group,
    { grain: { intensity: 0.08, size: 1.2, seed: frame }, opacity: toOut },
    h(Rect, {
      x: 0,
      y: 0,
      width: W,
      height: H,
      gradient: fbmGradient(CLOUD, { scale: 0.42, warp: 0.6, time: (t - 43) * 0.06 + 2.5 }),
    }),
    ...
    h(
      ...
      h(Rect, {
        ...
        fill: WHITE,
        cornerRadius: 14,
        shadow: { color: '#00000030', blur: 24, offsetY: 8 },
      }),
      h(
        Text,
        { x: wcx + 16, y: wcy + 16, fontSize: 16, fontFamily: SANS, fontWeight: 600, color: FG },
        'Flows Agent',
      ),
      ...
      h(Rect, {
        x: wcx + 12,
        y: wcy + 44,
        width: WHAT_CARD_W - 24,
        height: 26,
        fill: '#F4F4F8',
        cornerRadius: 8,
      }),
      h(FadeType, {
        text: 'What should we create next?',
        x0: wcx + 20,
        y: wcy + 52,
        size: 14,
        weight: 400,
...
function SceneLogo({ frame }) {
  ...
  return h(
    Group,
    { grain: { intensity: 0.08, size: 1.2, seed: frame } },
    h(Rect, {
      x: 0,
      y: 0,
      width: W,
      height: H,
      gradient: fbmGradient(CLOUD, { scale: 0.42, warp: 0.6, time: (t - 46) * 0.06 + 3.2 }),
    }),
...
function FlowsFull({ width, height }) {
  ...
  return h(
    ...
    h(AudioClip, {
      key: 'audio',
      src: '/Users/rodrigosilva/dev/onda-engine/refs/flows-full-audio.aac',
      volume: 0.85,
    }),
  )
}
