...
const PLATE = DIR + '/aura/plate.jpg',
  GLOW = DIR + '/aura/glow.jpg',
  NOIR = DIR + '/aura/noir.jpg',
  EMBER = DIR + '/aura/ember.jpg'
const AUDIO = DIR + '/aura-amb.wav' // warm ambient pad + synthesized SFX (whoosh / chimes / impact)

const win = (f, a, b, c, d) =>
  f < a ? 0 : f < b ? (f - a) / (b - a) : f < c ? 1 : f < d ? 1 - (f - c) / (d - c) : 0
const lerp = (f, a, b, va, vb) => {
...
const ColdOpen = (f) => {
  ...
  return h(
    ...
    h(
      ...
      h(
        Text,
        { fontSize: 28, fontFamily: 'Spectral', color: '#b59a6e', letterSpacing: 12 },
  ...
  return h(
    ...
    h(
      ...
      h(
        Center,
        { y: 1500, height: 90 },
        h(Text, { fontSize: 60, fontFamily: 'Bricolage Grotesque 96pt', color: '#f3ecdb' }, 'AURA'),
      ),
      h(
        Center,
        { y: 1606, height: 60 },
        h(
          Text,
          { fontSize: 25, fontFamily: 'Spectral', color: '#d2c4ab', letterSpacing: 6 },
  ...
  return h(
    ...
    h(
      ...
      h(
        ...
        h(
          Text,
          { fontSize: 96, fontFamily: 'Bricolage Grotesque 96pt', color: '#f3e6cf' },
          seg.word,
        ),
      ),
    ),
...
const Reveal = (f) => {
  ...
  return h(
    ...
    h(Rect, {
      width: W,
      height: H,
      gradient: radialGradient([CX, 920], 980, [
        { offset: 0, color: '#2b2114' },
        { offset: 0.5, color: '#130d0a' },
        { offset: 1, color: '#070608' },
      ]),
    }),
    h(Ellipse, {
      width: 1100,
    ...
    h(
      Scene3D,
      { camera: { position: [CX, 920, -2600], fov: 34 } },
      h(
        Text,
        {
          fontSize: 230,
    ...
    h(
      ...
      h(
        ...
        h(
          Text,
          { fontSize: 30, fontFamily: 'Spectral', color: '#b69a72', letterSpacing: 10 },
...
const comp = h(
  Composition,
  {
    ...
    finish: {
      bloom: { sigma: 13, threshold: 0.7, intensity: 0.95 },
      halation: 0.28,
      temperature: 0.16,
      contrast: 1.1,
      saturation: 1.05,
