...
with `@onda-engine/react`. Read it before authoring; it covers the full node surface,
...
`useAudioBeats(src)` (from `@onda-engine/components`) analyzes an audio clip into
`{ tempo, beats, onsets, onsetEnv }` — **all in frame units** — using the same
`@onda-engine/wasm-audio` as the FFT, so it's deterministic and identical in preview and
...
## `@onda-engine/components` — ready-made animation primitives

Import from `@onda-engine/components`. These wrap `Group` + `spring`/`interpolate` in
