...
`@onda/react` provides two ways to drive values from the frame: `interpolate` (map a frame through input/output ranges) and `spring` (natural, physics-based motion). Both are pure functions, so renders are reproducible.
...
```tsx
import { spring, useCurrentFrame, useVideoConfig } from '@onda/react'
