...
export const ENGINE_CAPABILITIES = {
  supported: [
    ...
    'precomp / adjustment layers — `<Precomp>` flattens a subtree to ONE layer (render-to-texture) so its opacity/blend apply to the COMPOSITED result, not per-child (fixes fading-a-group double-darkening overlaps; AE precomp / collapse-transformations); an adjustment layer = a `<Group>` with an effect wrapping the layers below it. Both backends',
