...
export const codeDiffSchema = z.object({
  ...
  fontFamily: z.string().describe("Monospace font stack for code and title (defaults to theme monoFamily ?? fontFamily)."),
  fontSize: z.number().default(44).describe("Code font size in px."),
  width: z.number().default(760).describe("Panel width in px."),
  textColor: z.string().describe("Default (context) text color (defaults to theme textMuted)."),
  addColor: z.string().describe("Added-line color (defaults to theme palette[3])."),
  removeColor: z.string().describe("Removed-line color (defaults to theme accent)."),
  surfaceColor: z.string().describe("Panel surface (glass) fill (defaults to theme surface)."),
  borderColor: z.string().describe("Panel border / chrome divider color (defaults to theme border)."),
  cornerRadius: z.number().describe("Panel corner radius in px (defaults to theme radius)."),
  chromeDotsColor: z.string().describe("Window-chrome traffic-light dot color (defaults to theme border)."),
  chromeTitleColor: z.string().describe("Window-chrome title (filename) color (defaults to theme textMuted)."),
})
