...
export const codeDiffSchema = z.object({
  ...
  addColor: z.string().optional().describe("Added-line color, a restrained green (defaults to theme palette[3])."),
  removeColor: z.string().optional().describe("Removed-line color, a restrained red tinted toward bg (defaults to #cf6f7e)."),
  ...
  chromeTitleColor: z.string().optional().describe("Window-chrome title (filename) color, the one earned accent (defaults to theme accent)."),
})
