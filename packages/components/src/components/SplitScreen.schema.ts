...
export const splitScreenSchema = z.object({
  ...
  ratio: z.number().default(0.5).describe("Fraction (0..1) of the main axis given to the left (or top) pane."),
  ...
  background: z.string().optional().describe("Outer (gutter) background fill, seen in the gap behind the divider (default: theme background)."),
  dividerColor: z.string().optional().describe("Divider color (thin token line) (default: brighter hairline edge color)."),
  placeholderColor: z.string().optional().describe("Placeholder label color for an empty pane (default: theme textMuted)."),
  fontFamily: z.string().optional().describe("Loaded font family for empty-pane placeholders (default: theme fontFamily)."),
})
