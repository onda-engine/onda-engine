...
export const staggerGroupSchema = z.object({
  items: z
    ...
    .describe('The items to reveal, in source order.'),
  delay: timeSchema.default(0).describe('Frames before the first item starts.'),
  stagger: timeSchema
    .optional()
    .describe('Frames between consecutive items; canonical Onda stagger is 4.'),
  duration: timeSchema.optional().describe('Per-item reveal duration in frames.'),
  direction: z
    .enum(['row', 'column'])
    .default('column')
    .describe('Layout direction for the items.'),
