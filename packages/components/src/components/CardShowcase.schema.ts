...
export const cardShowcaseSchema = z.object({
  brand: z.string().default('Lumen').describe('Brand wordmark shown big on the hero card + as the centered logo.'),
  network: z.string().default('VISA').describe('Payment-network mark on the hero card (e.g. VISA, Mastercard).'),
  ...
  tilt: z.number().default(-22).describe('Grid tilt in degrees (the diagonal alignment); rotates to 0 at the end.'),
  speed: z.number().default(320).describe('Slide speed in px/sec.'),
  logo: z.string().default('').describe('Center logo text revealed at the end (e.g. a brand). Empty = none.'),
