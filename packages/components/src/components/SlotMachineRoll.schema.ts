...
export const slotMachineRollSchema = z.object({
  ...
  charDelay: timeSchema.default(5).describe("Frames between successive characters starting their roll (default the house STAGGER = 5 - a settled, orchestrated wave left-to-right)."),
  durationInFrames: timeSchema.optional().describe("Frames for each character's reel to settle (default DURATION.slower = 34 - a slow, hard-decelerating odometer drop, not a constant-velocity spin)."),
