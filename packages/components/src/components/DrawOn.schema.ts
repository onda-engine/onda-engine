...
export const drawOnSchema = z.object({
  ...
  delay: z.number().int().default(0).describe("Frames before the draw-on starts."),
  durationInFrames: z.number().int().optional().describe("Frames to fully draw the path in (default `DURATION.slow` = 24)."),
})
