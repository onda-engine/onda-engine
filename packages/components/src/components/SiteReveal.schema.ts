...
export const siteRevealSchema = z.object({
  ...
  pageAspect: z
    .number()
    .default(4.48)
    .describe('Page height / width of src (drives the scroll extent).'),
  ...
  offsetY: z
    ...
    .default(56)
    .describe(
      'Vertical nudge of the card from center (slightly low to leave room for a headline above).',
    ),
  delay: z.number().int().default(0).describe('Frames before the card enters.'),
  typeUrl: z
    .boolean()
    .default(true)
    .describe('Type the URL into the address bar (with a blinking cursor) before scrolling.'),
  typeDurationInFrames: z.number().int().default(26).describe('Frames the URL takes to type in.'),
  scrollStart: z
    .number()
    .default(0)
    .describe('Page fraction (0..1) shown at the start of the scroll.'),
  scrollEnd: z.number().default(0.62).describe('Page fraction (0..1) reached at the end.'),
  scrollDurationInFrames: z
    .number()
    .int()
    .default(150)
    .describe('Frames over which the scroll completes.'),
  ...
  border: z
    .string()
    .optional()
    .describe('1px card border + chrome divider (defaults to theme border).'),
