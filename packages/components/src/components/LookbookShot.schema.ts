...
export const lookbookShotSchema = z.object({
  src: z.string().default('').describe('Product photo source (resolved at render time by `onda render`).'),
  name: z.string().default('Product').describe('Product name — the headline, set in the display face.'),
  eyebrow: z
    .string()
    .default('')
  ...
  layout: z
    ...
    .describe('Page composition. Alternate spread-right / spread-left across shots for lookbook rhythm.'),
  ...
  matColor: z.string().optional().describe('The mat/frame color (the print border). Default a near-white off the bg.'),
  ...
  fontFamily: z.string().optional().describe('Display font for the name (defaults to theme headingFamily ?? fontFamily).'),
  bodyFamily: z.string().optional().describe('Body font for eyebrow + detail (defaults to theme fontFamily).'),
