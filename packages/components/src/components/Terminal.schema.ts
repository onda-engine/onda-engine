...
export const terminalSchema = z.object({
  ...
  command: z
    ...
    .describe('The command that types itself out after the prompt.'),
  output: z
    .array(z.string())
    .default(['✓ added code-block', '✓ wrote 4 files'])
    .describe('Output lines that appear, staggered, once the command finishes typing.'),
  prompt: z.string().default('$').describe('The shell prompt glyph.'),
  title: z
    .string()
    .default('zsh')
    .describe('Title-bar label; empty hides it (dots still show if chrome is on).'),
  ...
  typeSpeed: timeSchema.default(30).describe('Frames to type the whole command (linear cadence).'),
  ...
  promptColor: z
    .string()
    .optional()
    .describe('Prompt glyph color \u2014 the earned accent (default: theme accent).'),
  ...
  cornerRadius: z
    .number()
    .optional()
    .describe('Window corner radius in px (default: theme radius).'),
  x: z
    .number()
    .optional()
    .describe("Absolute x of the window's top-left; defaults to horizontally centered."),
  y: z
    .number()
    .optional()
    .describe("Absolute y of the window's top-left; defaults to vertically centered."),
