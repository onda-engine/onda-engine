...
export const browserFrameSchema = z.object({
  ...
  from: z.number().default(0.96).describe("Starting scale for the entrance (restrained, like ondajs)."),
  surface: z.string().optional().describe("Card body fill (default: theme surface)."),
  border: z.string().optional().describe("1px card border and chrome divider color (default: theme border)."),
  ...
  pillRadius: z.number().optional().describe("Address-pill corner radius in px (default: theme radius)."),
})
