...
export const meshGradientSchema = z.object({
  ...
  background: z
    .string()
    .optional()
    .describe('Base canvas color behind the blobs (defaults to theme background).'),
  speed: z
    .number()
    .default(1)
    .describe('Drift speed multiplier; keep low since this is atmosphere, not motion.'),
  seed: z
    .number()
    .int()
    .default(7)
    .describe('Seed for the blob phase/amplitude offsets (deterministic).'),
