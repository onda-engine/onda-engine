...
export const textFadeReplaceSchema = z.object({
  ...
  delay: timeSchema.optional().describe("Frames before the crossfade begins; until then only `from` is shown (default 45)."),
  durationInFrames: timeSchema.optional().describe("Frames the crossfade takes \u2014 old out over the first half, new in over the second (default 18)."),
