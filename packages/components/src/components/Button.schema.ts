...
export const buttonSchema = z.object({
  ...
  centerY: z
    .number()
    .default(0.5)
    .describe('Vertical center as a 0\u20131 fraction of canvas height (default 0.5).'),
  entrance: z
    .boolean()
    .default(true)
    .describe('Play the entrance (fade + rise on the house spring).'),
  delay: timeSchema.default(0).describe('Frames before the entrance begins.'),
  durationInFrames: timeSchema
    .optional()
    .describe('Entrance duration in frames (default DURATION.base = 18).'),
  press: z.boolean().default(true).describe('Play the click-dip press animation.'),
  pressFrame: timeSchema
    .default(30)
    .describe('Frame the press dip lands on (relative to the local timeline).'),
