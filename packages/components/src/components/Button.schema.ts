...
export const buttonSchema = z.object({
  ...
  color: z.string().describe("Accent color \u2014 the primary fill and the ghost border/label tint (default: theme accent)."),
  textColor: z.string().describe("Label color on the primary variant (default: theme text). Ignored by 'ghost', which tints the label with color."),
  width: z.number().default(280).describe("Pill width in px."),
  height: z.number().default(72).describe("Pill height in px."),
  cornerRadius: z.number().describe("Corner radius in px (default: theme radius)."),
  ...
  fontFamily: z.string().describe("Loaded font family (e.g. a --font passed to onda render) (default: theme fontFamily)."),
