...
export const barChartSchema = z.object({
  ...
  max: z
    .number()
    .default(100)
    .describe('Value mapped to a full-width bar. Bars cap at 100% of the track.'),
  delay: timeSchema.default(0).describe('Frames before the first bar starts.'),
  duration: timeSchema
    .optional()
    .describe('Per-bar grow duration in frames. Bars want more time than text (default slow).'),
  stagger: timeSchema
    .optional()
    .describe('Frames between consecutive bars (canonical STAGGER = 4).'),
  barHeight: z.number().default(32).describe('Bar (and track) height in px.'),
  gap: z.number().default(16).describe('Pixel gap between rows.'),
  labelWidth: z
    .number()
    .default(220)
    .describe('Pixels reserved for the label column (left of the track).'),
  labelGap: z.number().default(24).describe('Gap between the label column and the track, in px.'),
  trackWidth: z
    .number()
    .default(760)
    .describe('Track length in px \u2014 the full-width target for a bar at max.'),
  accentColor: z
    .string()
    .optional()
    .describe('Color of the largest bar \u2014 the earned accent. Defaults to theme.accent.'),
  barColor: z
    .string()
    .optional()
    .describe('Color of non-largest bars. Defaults to theme.palette[0] (or theme.textMuted).'),
  trackColor: z
    .string()
    .optional()
  ...
  title: z
    .string()
    .optional()
    .describe('Optional headline above the chart \u2014 tells viewers what the numbers measure.'),
  titleSize: z
    .number()
    .optional()
