...
export const videoClipSchema = z.object({
  ...
  startAt: z.number().default(0).describe("Seconds into the source shown at the clip's frame 0 (trims the head)."),
  playbackRate: z.number().default(1).describe("Source seconds advanced per composition second (1 = realtime)."),
  ...
  borderRadius: z.number().optional().describe("Rounded corner radius of the box in px (defaults to theme radius)."),
  letterbox: z.number().default(0).describe("Cinematic black letterbox bars top & bottom, each this many px tall (0 = none)."),
  backgroundColor: z.string().optional().describe("Backing color shown behind/around the poster (defaults to theme background)."),
})
