    ...
    <Group>
      {[...text].map((ch, i) => {
        ...
        const scale = interpolate(
          spring({ frame: t, fps, config: { stiffness: 210, damping: 13 } }),
          [0, 1],
          [0.6, 1],
        )
        ...
        return (
          <Group
            // biome-ignore lint/suspicious/noArrayIndexKey: letters of a static string never reorder
            key={i}
            x={slotCenterX}
            y={baseline + rise}
            scaleX={scale}
...
function Underline({
  ...
  width,
  delay,
}: { cx: number; y: number; width: number; delay: number }) {
  const frame = useCurrentFrame()
  const { fps } = useVideoConfig()
...
function Tagline({
  ...
  fontSize,
  delay,
}: { text: string; cx: number; y: number; fontSize: number; delay: number }) {
  const frame = useCurrentFrame()
  const t = frame - delay
